...
export const recordingService = {
  async createRecording(req: CreateRecordingRequest): Promise<Recording> {
    ...
    const newRecording: Recording = {
      ...
      // Undefined when the trainer uploaded without mapping to the curriculum.
      moduleId: req.moduleId,
      moduleTitle: req.moduleId ? "Module " + req.moduleId : undefined,
      lessonId: req.lessonId,
      lessonTitle: req.lessonId ? "Lesson " + req.lessonId : undefined,
      teacherId: "u-teacher",
      teacherName: "Durga Prasad",
      title: req.title,
      description: req.description ?? "",
      classDate: req.classDate ?? new Date().toISOString(),
      durationSeconds: 0,
      fileSizeBytes: 0,
      status: "DRAFT",
      published: false,
