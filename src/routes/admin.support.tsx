  ...
  head: () => ({
    meta: [
      { title: "Support Desk — Learntrix Admin" },
      {
        name: "description",
        content:
          "Triage, assign and resolve student support tickets with internal staff notes and escalation.",
      },
      { property: "og:title", content: "Support Desk — Learntrix Admin" },
      {
        property: "og:description",
        content: "Ticket triage, assignment, escalation and resolution tracking.",
      },
...
function AdminSupport() {
  ...
  return (
    <>
      <PageHeader
        title="Support Desk"
        description="Ticket permissions, visibility and escalation paths are enforced by the backend."
      />
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <StatCard
          ...
          value={overview.data ? `${overview.data.averageResolutionHours}h` : "—"}
        />
      </section>
                    ...
                    <TableCell>
                      <Select
                        ...
                        onValueChange={async (v) => {
                          await supportService.updateStatus(t.id, v as TicketStatus);
                          refresh();
                      ...
                      >
                        <SelectTrigger
                          ...
                          aria-label={`Status of ${t.reference}`}
                        >
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
...
function TicketThread({ id, onChanged }: { id: string; onChanged: () => void }) {
  const ticket = useQuery({
    queryKey: ["support", "ticket", id, "staff"],
    queryFn: () => supportService.detail(id, "staff"),
  });
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>
          {t.subject}{" "}
          <span className="ml-2 text-xs font-normal text-muted-foreground">{t.reference}</span>
        </DialogTitle>
      </DialogHeader>
      ...
      <Textarea
        rows={3}
        placeholder={
          internal ? "Internal note — not visible to the student" : "Reply to the student"
        }
        value={body}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button
            size="sm"
            onClick={async () => {
              await supportService.reply(t.id, body, {
                internal,
                author: "Support Desk",
                authorRole: "staff",
              });
