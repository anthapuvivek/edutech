...
export const Route = createFileRoute("/admin/career/referrals")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Review, approve and track alumni and partner referral requests raised by students." },
...
function AdminReferrals() {
  const referrals = useQuery({ queryKey: ["admin", "referrals"], queryFn: () => adminService.referrals() });
  ...
  return (
    <>
      <PageHeader title="Referrals" description="Referral requests never reach a company until an admin approves them." />

      <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Requests" value={data.length} />
        <StatCard label="Under review" value={data.filter((r) => r.status === "Under Review" || r.status === "Requested").length} />
        <StatCard label="Approved" value={data.filter((r) => r.status === "Approved" || r.status === "Referred").length} />
              ...
              <TableBody>
                {data.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-sm">{new Date(r.requestedAt).toLocaleDateString()}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="space-x-2 text-right">
                      ...
                      <Button size="sm" variant="outline" onClick={() => void update(r.id, "Rejected")}>
                        Reject
                      </Button>
                    </TableCell>
                  </TableRow>
