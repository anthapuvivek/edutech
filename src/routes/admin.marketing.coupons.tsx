...
export const Route = createFileRoute("/admin/marketing/coupons")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Create, schedule and analyse promotional campaigns across courses and batches." },
      { property: "og:title", content: "Coupons & Promotions — Learntrix Admin" },
      { property: "og:description", content: "Promotional campaign management with usage and revenue analytics." },
...
function AdminCoupons() {
  ...
  const overview = useQuery({ queryKey: ["admin", "coupons", "overview"], queryFn: () => couponService.overview() });
  const [open, setOpen] = useState(false);

  const invalidate = () => void qc.invalidateQueries({ queryKey: ["admin", "coupons"] });

  const mutate = useMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Partial<Coupon> }) => couponService.update(id, patch),
    onSuccess: () => {
      invalidate();
  ...
  return (
    <>
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <StatCard label="Revenue generated" value={overview.data ? inr(overview.data.revenueGenerated) : "—"} />
        <StatCard label="Discount given" value={overview.data ? inr(overview.data.discountGiven) : "—"} />
      ...
      <Tabs defaultValue="campaigns">
        ...
        <TabsContent value="campaigns">
          <Panel title="All campaigns" description="Draft, schedule, activate, disable or expire promotional codes.">
            {coupons.isPending ? (
                    ...
                    {data.map((c) => (
                      <TableRow key={c.id}>
                        ...
                        <TableCell>
                          ...
                          {c.maximumDiscount ? <div className="text-xs text-muted-foreground">max {inr(c.maximumDiscount)}</div> : null}
                        </TableCell>
                        <TableCell className="text-xs text-muted-foreground">
                          {c.applicableCourses.length ? `${c.applicableCourses.length} course(s)` : "All courses"}
                          <div>{c.applicableBatches.length ? `${c.applicableBatches.length} batch(es)` : "All batches"}</div>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Select onValueChange={(v) => void act(c, v)}>
                            <SelectTrigger className="ml-auto w-[140px]" aria-label={`Actions for ${c.code}`}>
        ...
        <TabsContent value="usage">
          <Panel title="Coupon usage" description="Usage, revenue, discount and conversion per campaign.">
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
...
function CouponDialog({ onCreated }: { onCreated: () => void }) {
  const [form, setForm] = useState<Partial<Coupon>>({ discountType: "percentage", status: "draft", usageLimit: 100, usagePerStudent: 1 });
  ...
  return (
    <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription>The backend validates and applies these rules at checkout.</DialogDescription>
      </DialogHeader>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Coupon name">
          <Input value={form.name ?? ""} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Welcome offer" />
        </Field>
        ...
        <Field label="Maximum discount (₹)">
          <Input type="number" value={form.maximumDiscount ?? ""} onChange={(e) => setForm({ ...form, maximumDiscount: Number(e.target.value) })} />
        </Field>
        ...
        <Field label="Applicable courses (comma separated, blank = all)" className="sm:col-span-2">
          <Input onChange={(e) => setForm({ ...form, applicableCourses: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} />
        </Field>
        <Field label="Applicable batches (blank = all)">
          <Input onChange={(e) => setForm({ ...form, applicableBatches: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} />
        </Field>
        <Field label="Applicable users (blank = all)">
          <Input onChange={(e) => setForm({ ...form, applicableUsers: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} />
        </Field>
